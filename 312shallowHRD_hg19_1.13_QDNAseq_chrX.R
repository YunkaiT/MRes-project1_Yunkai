...
output_files <- list(
  "CNA_cut-off_value.txt" = THR,
  "Number_simulations_for_cut-off.txt" = nb_simulations,
  "Number_successfull_simulations.txt" = number_positive,
  "MAX2.txt" = round(MAX2, 3),
  "cMAD.txt" = QC_MAD_point_corrected,
  "Number_LGAs_10Mb.txt" = number_LGAs,
  "HRD.txt" = HRD
)


for (file_name in names(output_files)) {
  file_path <- paste0(outputPath, "/", file_name) 
  write.table(output_files[[file_name]], file = file_path, row.names = FALSE, col.names = FALSE, quote = FALSE)
}
